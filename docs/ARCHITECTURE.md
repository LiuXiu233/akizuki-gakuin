...
| 预设 | 阶段 | 每回合模型调用 | 特点 |
|---|---|---|---|
| `single` | turn | 1~N | 带完整 AGENT.md，一个 Agent 干完所有事 |
...
以下是**真机实测**（DeepSeek v4-pro，关闭推理，同一个存档、同一句输入）：

| 流水线 | 一回合耗时 | 一回合 token | 叙事长度 | NPC 台词分离 |
|---|---|---|---|---|
| `single` | ~24 秒 | ~85,000 | 短 | 否（混在正文里） |
| `dual` | ~23 秒 | ~46,000 | 中 | 是 |
| `multi` | ~45 秒 | ~60,000 | 长、细节最多 | 是（每人独立视角） |

**和直觉相反：`single` 最贵。** 因为它带完整 AGENT.md 且拿到全部 51 个工具，
而 system prompt 会在每一轮工具迭代里重发。`dual` 才是性价比最高的日常选择；
重要场景（告白、文化祭、修学旅行）再切 `multi`。

开销的大头不是工具说明（51 个加起来才 ~800 token），而是**工具返回值随迭代累积**。
执行器已经把回灌的结果截断到 2600 字符，并把解析阶段的工具收窄到四个。
...
Next.js 15 App Router + TypeScript + Tailwind + zustand，部署 Vercel。

### 两种界面模式（一键切换）

* **沉浸模式**：场景图 / 标题卡 → 立绘 → 底部对话框 → 推荐行动 → 输入框。
  面板以弹层形式按需打开。
* **面板模式**：左栏角色（状态 / 属性 / 技能 / 知识 / 社团），中栏叙事流与输入，
  右栏在场人物 / 图鉴 / 地图 / 日程 / 日志 / 调试 + 存档点。

### 面板

| 面板 | 内容 |
|---|---|
| 角色 | 健康 / 精力 / 压力 / 心情 / 金钱、7 项属性与修正、技能与知识等级、社团 |
| 在场 | 当前地点的人，点开看公开信息与关系标签 |
| 图鉴 | 所有认识的人的头像墙 |
| 地图 | 校内 / 校外分区，显示是否开门与步行时间，点击即移动 |
| 日程 | 日期时刻天气学期、今日课表、进行中的学年活动 |
| 日志 | 逐回合回放，含判定与成长区块 |
| 调试 | 每个 Agent 的耗时 / token / 真实工具调用（设置里开启） |

### 三种 LLM 调用位置

| 模式 | key 在哪 | 支持的流水线 |
|---|---|---|
| 自建后端 | 随请求发给后端，用完即弃；或用后端环境变量 | 全部 |
| Vercel 边缘 | Vercel 环境变量，你的服务器上没有 key | 全部 |
| 浏览器直连 | 只在这台设备 | 仅单 Agent（编排在浏览器里做） |

### 两个已知坑与对策

1. **https 页面 + http 后端**：浏览器拦截混合内容 → 设置里切「经 Vercel 转发」，
   请求走 `/api/proxy`，由 Next.js 服务端转发。
2. **持久化设置的水合竞态**：刷新后首帧拿到的是默认值（令牌为空）。
   前端用 `useHydrated()` 等待恢复完成，并在异步回调里用
   `useSettings.getState()` 现取，避免闭包捕获到旧值。

---

## 7. 文生图

四类图像，**全部按需生成 + 本地缓存 + 按用户完全隔离**：

| 类型 | 用途 | 提示词来源 |
|---|---|---|
| `avatar` | 列表、在场人物、图鉴的方形头像 | NPC 的 `appearance` 字段 + 制服描述 |
| `portrait` | 沉浸模式的立绘 | 同上，三分身站姿 |
| `scene` | 当前地点的背景插画 | 地点 `description` + 时刻 / 天气 / 季节 |
| `cg` | 重要剧情节点的插画 | 画师 Agent 生成的英文提示词 |

**不配置就整体关闭**：接口返回 `skipped`，前端自动退回按名字生成的配色占位符，
不会有任何报错弹窗，也不会请求任何外部服务。

### 两种上游

* `openai`：`POST {base_url}/images/generations`，解析 `data.0.b64_json` 或 `data.0.url`
* `custom`：你自己写请求体模板与取图路径

```jsonc
// 请求体模板（{prompt} / {model} / {size} 会被替换）
{"prompt": "{prompt}", "steps": 28, "width": 1024, "height": 1024}
// 取图路径
"images.0"        // 也可以是 data.0.b64_json、output.0.url 等
```

响应值是 URL 就下载，是 base64 就解码，`Content-Type: image/*` 就直接当图片。

### 两条硬约束

1. **所有人物提示词强制标注 adult (18+)**。这个世界里不存在未成年角色，图像也一样。
2. **默认强制 SFW**：加上着装与日常场景约束，并从提示词里剔除风险词。
   服务器把 `AKIZUKI_IMAGE_SFW=true`（默认）时，客户端**无法**关掉它。
   这个开关只影响图像——文字叙事的尺度完全不受影响。

### 前端的一个坑

图片接口需要鉴权头（访问口令 + 用户令牌），而 `<img src>` 发不了自定义头。
前端统一用 `fetch` 拉成 blob 再显示（`lib/assets.ts`），同一路径只请求一次。
...
- [x] **M3 Next.js 前端** — 双模式界面、四类面板、设置、流式渲染、浏览器直连模式
- [x] **M4 文生图** — 四类图像、按需生成、缓存、自定义模板、SFW 约束、按用户隔离
